...
# v0.1 2011-07-08
# Script to read an aliases file (two columns of names) and replace every instance, in another file, of the name in one column with the name in the other column.
...
def swap_words(current_words, new_words, file):
	...
	for index in range(len(current_words)):
		file_string = file_string.replace(current_words[index], new_words[index])
	return file_string
...
def main():
	...
	p.add_option('-c', '--column', default = 2, type = 'int', help = "which column contains the names to be switched in [default: %default]")
